...
The "best" model selection scheme chooses the top-performing model/LASSO parameter on the holdout dataset from the same source as the training data, and applies it to the test data.
...
We saw that for TCGA to CCLE generalization, 27/71 genes (38.0%) had better performance for the "best" model and 17/71 genes (23.9%) had better generalization performance with the "smallest good" model, with the other 27 genes having the same "best" and "smallest good" model (in other words, the "smallest good" model was also the best-performing overall, and the difference was exactly 0) (Figure {@fig:tcga_ccle_smallest_best}B).
...
Overall, these results do not support the hypothesis that the most parsimonious model generalizes the best: for both generalization problems there are slightly more genes where the best-performing model on the holdout dataset is also the best-performing on the test set, although there are some genes where the "smallest good" approach does work well.

We looked at examples of genes that fall into either category for TCGA to CCLE generalization (dotted lines on Figure {@fig:tcga_ccle_smallest_best}B).
_NF1_ is an example of a gene where the "best" model outperforms the "smallest good" model (Figure {@fig:tcga_ccle_smallest_best}D).
