...
For experiments making predictions across datasets (i.e. training models on TCGA and evaluating performance on CCLE, or vice-versa) we took the intersection of gene expression features in both datasets, resulting in 16041 gene features.
...
In order to study mutation status classification for a diverse set of cancer driver genes, we started with the set of 124 frequently altered genes from Vogelstein et al. [@doi:10.1073/pnas.1616440113] (all genes from Table S2A).
...
Out of the 125 genes originally listed in the Vogelstein et al. cancer gene set, we retained 70 target genes for the TCGA to CCLE and CCLE to TCGA analyses.
For the cancer type holdout analysis, we retained 56 target genes: in this case each gene needs at least two valid cancer types to be retained, one to train on and one to hold out.
